...
void AXBOpenGLWidget::paintGL()
{
    ...
    glEnable(GL_CULL_FACE); // Bật chế độ cull face
    // Thiết lập mặt trước là CW(Theo chiều kim đồng hồ) CCW(Ngược chiều kim đồng hồ)
    glFrontFace(GL_CCW);
    // mặc định là GL_BACK: Không hiển thị mặt sau,chỉ hiển thị mặt trước.
    // GL_FRONT           : Không hiển thị mặt trước, Chỉ hiển thị mặt sau.
    // GL_FRONT_AND_BACK  : Không hiển thị mặt trước lẫn mặt sau.
    glCullFace(GL_BACK);
    ...
    // Ngược chiều kim đồng hồ
    model.translate(3.0f,-3.0f,0.0f);
    shaderProgram.setUniformValue("model", model);
    CKDHMesh->Draw(shaderProgram);
